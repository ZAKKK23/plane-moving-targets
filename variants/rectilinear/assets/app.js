...
function buildTargetRows(targets) {
  ...
  targets.forEach((tg, k) => {
    ...
    row.innerHTML = `
      ...
      <div class="ctrl">Spd:
        <input type="range" min="0" max="1" step="0.001" value="${tg.spd}" data-k="${k}" data-field="spd" />
        <span class="readout" data-readout="spd-${k}">${tg.spd.toFixed(3)}</span>
      </div>
      <div class="ctrl">Ang(deg):
        <input type="range" min="0" max="360" step="0.1" value="${(tg.heading * 180 / Math.PI).toFixed(1)}" data-k="${k}" data-field="heading" />
        <span class="readout" data-readout="heading-${k}">${(tg.heading * 180 / Math.PI).toFixed(1)}</span>
      </div>`;
  ...
  targetRows.querySelectorAll("input[type=range]").forEach((inp) => {
    inp.addEventListener("input", () => {
      ...
      const val = +inp.value;
      const tg = sim.targets[k];
      if (field === "heading") {
        tg.heading = (val * Math.PI) / 180;
      } else {
        tg.spd = val;
      }
      sim.syncTargetVelocity(tg);
      const ro = targetRows.querySelector(`[data-readout="${field}-${k}"]`);
      ro.textContent = field === "spd" ? val.toFixed(3) : val.toFixed(1);
    });
  });
}
...
function loop() {
  sim.step();
  sim.draw();
  statusLine.textContent = `t = ${sim.t} | ${sim.targets.length} moving target${sim.targets.length === 1 ? "" : "s"} (rectilinear + bounce) | \u03bd = ${sim.nu.toFixed(2)}, L = ${sim.L.toFixed(2)}`;
