...
function loop() {
  ...
  sim.draw();
  const nRect = sim.nRect;
  statusLine.textContent = `t = ${sim.t} | ${sim.targets.length} aligned target${sim.targets.length === 1 ? "" : "s"} (${nRect} straight + 1 asym. sine) | vx = ${sim.vx.toFixed(3)}, \u03bd = ${sim.nu.toFixed(2)}, L = ${sim.L.toFixed(2)}`;
