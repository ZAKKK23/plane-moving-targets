...
%      sinusoidally in y, but ASYMMETRICALLY — it goes down at angular
%      frequency q ("Omega") and comes back up at frequency n*q ("n"
%      times faster), so one full oscillation is a slow descent
%      followed by a quick rise. n is itself a live-editable parameter.
%      Both halves are still genuine sine curves (a smooth half-cosine
%      each), just run at two different rates, joined continuously at
%      the top and bottom of the swing.
...
%      amplitude, down-frequency q ("Omega"), and the up-speed
%      multiplier n
...
while t < tmax && ishandle(fig)
    ...
    if ishandle(fig)
        ...
        title(ax, sprintf('t = %d    |    %d aligned targets (%d straight + 1 asym. sine)    |    vx = %.3f, q = %.3f, n = %.2f, \nu = %.2f, L = %.2f', ...
              t, nT, nRect, cur_vx, cur_Omega, cur_n, cur_nu, cur_L), 'FontSize', 10);
        drawnow limitrate;
    end
