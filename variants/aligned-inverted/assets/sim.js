/* ===========================================================
   VTP — Aligned Moving Targets (inverted)  |  live simulation
   ...
   build. This is the inverse of the earlier "aligned" version: there,
   you chose a number of straight-line targets and one oscillating
   target was always added; here, you choose a number of OSCILLATING
   targets (each with its own independent height/amplitude/omega) and
   one STRAIGHT-LINE target is always added on top. ALL targets share
   a single horizontal speed (vx) and a single shared x-coordinate, so
   they are aligned (same x at every instant) by construction, not
   just at t=0.
   =========================================================== */
...
const PARAMS = {
  ...
  HEIGHT_RANGE: 8,     // oscillating targets' centers are spaced across [-HEIGHT_RANGE, HEIGHT_RANGE]
  OSC_AMP_DEFAULT: 4,
...
class VTPSim {
  ...
  // nOsc = number of independently-parametrized oscillating targets
  // (0-5). One more target — the straight-line one — is always added,
  // so total targets = nOsc + 1.
  setup(nOsc) {
    nOsc = clamp(Math.round(nOsc), 0, 5);
    this.nOsc = nOsc;
    const N = PARAMS.N;
    ...
    // ---- targets: nOsc independent oscillators + 1 straight line ----
    this.targets = [];
    const H = PARAMS.HEIGHT_RANGE;
    for (let k = 0; k < nOsc; k++) {
      const height = nOsc === 1 ? 0 : -H + (2 * H * k) / (nOsc - 1);
      this.targets.push({
        osc: true,
        height,                              // oscillation center — independent per target
        amp: PARAMS.OSC_AMP_DEFAULT,           // independent per target
        omega: PARAMS.OSC_OMEGA_DEFAULT,       // independent per target
        phase: 0,                              // independent per target
      });
    }
    this.targets.push({ osc: false, height: 0 });   // the single straight-line target
    this.rectIdx = this.targets.length - 1;
  ...
  step() {
    ...
    // construction, not just at t=0. Each oscillating target advances
    // its OWN phase at its OWN omega, independently.
  ...
  draw() {
    ...
    // targets: filled dot, velocity arrow, label ("T3~" for oscillating ones)
    const rC = 0.7 * scale;
